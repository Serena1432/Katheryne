...
module.exports = (client, interaction) => {
    ...
    if (interaction.user.id != userId) return interaction.reply({content: Language.strings.notTheAuthor.format(interaction.user.toString(), userId), components: [], embeds: [], ephemeral: true});
    if (!command || !command.config || !command.run) return interaction.reply({content: Language.strings.commandNotFound, ephemeral: true});
